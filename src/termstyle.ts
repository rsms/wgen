...
export function getStyle(s :NodeJS.WriteStream) {
  let ncolors = s.isTTY && s.hasColors() ? s.getColorDepth() : 0
  const sfn = (
    ncolors >= 8 ? (open16 :string, open256 :string, close :string) => {
      let a = '\x1b[' + open256 + 'm'
      let b = '\x1b[' + close + 'm'
      return (s :string) => a + s + b
    } :
    ncolors > 0 ? (open16 :string, _open256 :string, close :string) => {
      let a = '\x1b[' + open16 + 'm'
      let b = '\x1b[' + close + 'm'
      return (s :string) => a + s + b
    } :
    (_open16 :string, _open256 :string, _close :string) => (s :string) => s
  )
  return {
    ncolors,
    'reset'         : "\e[0m",
